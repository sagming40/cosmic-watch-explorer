...
| # | 마일스톤 | 범위 | 잠정 소요 | 상태 |
|---|---|---|---|---|
| M0 | 개발 환경 구성 | Django + MariaDB + Git | 3~5일 | ⬜ |
...
- [ ] Git 저장소 생성 및 `.gitignore` 작성 (`.env`, `venv/`, `node_modules/` 포함)
- [ ] `backend/` Django 프로젝트 생성 (`config` 설정 모듈)
- [ ] 가상환경 + `requirements.txt` (django, djangorestframework, mysqlclient, python-dotenv, requests)
- [ ] MariaDB `cosmic_watch` 데이터베이스 생성 (utf8mb4)
- [ ] `.env` / `.env.example` 작성 — `SECRET_KEY`, DB 접속 정보, `NASA_API_KEY`
- [ ] `settings.py` DB 설정 및 환경변수 로딩
- [ ] `frontend/` Vite + React 프로젝트 생성
- [ ] `vite.config.js` 프록시 설정 (`/api` → `localhost:8000`)
- [ ] NASA API 키 발급 및 `.env` 등록
- [ ] 문서 6종을 `docs/`에 배치하고 첫 커밋
### 완료 기준
 
- [ ] `python manage.py migrate` 가 오류 없이 끝나고, HeidiSQL에 `django_migrations` 등 기본 테이블이 보인다
- [ ] `python manage.py runserver` 후 브라우저에서 Django 기본 페이지가 뜬다
- [ ] `npm run dev` 후 `localhost:5173` 에서 React 페이지가 뜬다
- [ ] `git status` 에 `.env`가 나타나지 않는다
- [ ] 원격 저장소에서 `.env` 파일이 조회되지 않는다
...
- [ ] HeidiSQL에서 테이블 **9개**(Django 기본 제외)가 모두 확인된다
- [ ] `sqlmigrate`로 확인한 `CREATE TABLE` 문이 `02_database_design.md` 6장 DDL과 컬럼·제약 기준으로 일치한다
- [ ] shell에서 `fetch_feed('2026-08-21')` 실행 후, `close_approach` 행 수가 NASA 응답의 `element_count`와 **일치한다**
- [ ] 같은 날짜로 `fetch_feed`를 **두 번** 실행해도 `close_approach` 행 수가 늘어나지 않는다 (UNIQUE 제약 검증)
- [ ] `fetch_exoplanets` 실행 후 `exoplanet` 행 수가 5,000건 이상이다
- [ ] `exoplanet` 테이블에 `radius_earth IS NULL` 인 행이 **존재한다** (NULL을 0으로 바꾸지 않았음을 증명)
- [ ] Django Admin에서 소행성 목록이 조회된다
