...
이 엔드포인트와 5.2가 조건부로 NASA API를 호출한다. 각각 별도의 Throttle Scope
(`neo_fetch`, `neo_detail_fetch`)를 사용하여 소비량을 독립적으로 관리한다.
...
#### 설계 결정 ③ ─ `recent_approaches`는 "다음 접근 5건"이다

**"최근"이라는 이름과 달리, 과거가 아니라 미래 접근 기준이다.** 오늘(요청 시각) 이후
가장 가까운 예정 접근을 오름차순(가까운 것부터)으로 5건 내려준다.

이 서비스가 "감시(Watch)" 목적이라, 이미 지나간 접근보다 "다음에 언제 오는가"가
사용자에게 더 유용하다고 판단했다. 예정 접근이 5건 미만인 소행성은 있는 만큼만
내려준다 ─ 과거 접근으로 억지로 채우지 않는다. (NULL을 임의 값으로 바꾸지 않는
원칙과 같은 결, `02_database_design.md` 1.1절).

전체 접근 기록(과거 포함)을 보려면 `5.3 GET /api/neo/{nasa_id}/approaches/`을 사용한다.
...
`approach_datetime_utc` 오름차순(과거 → 미래) 고정이다. 사용자가 바꿀 수 있는
`sort` 파라미터는 존재하지 않는다. ─ "전체 기록을 훑어보는" 용도이므로, 연대기 처럼 고정된 순서로
읽히는 쪽을 택했다.
...
`nasa_id`에 해당하는 소행성 자체가 없을 때, "접근 기록 0건"과 구분짓기 위해
Neo 존재 여부를 먼저 확인한다. (5.2와 동일 원칙).
...
`sort`를 생략하거나 목록에 없는 값을 보내면 `name` 오름차순으로 조용히 대체된다. `sort`는 6.1의 "검색 조건"이 아니라 화면 표시 순서 조작값이라, `radius_min` 같은 검색 조건과 달리 값이 잘못돼도 400을 반환하지 않는다.
...
#### 설계 결정 ④ — 정렬 시 NULL은 항상 뒤로 보낸다

`distance_pc`, `radius_earth`, `mass_earth`는 전부 NULL을 허용하는 컬럼이다(`radius_earth IS NULL` 1,612건 — 문서 01, M1에서 확인). MariaDB의 기본 정렬은 오름차순일 때 NULL을 맨 앞으로 보내므로, "가까운 순"으로 정렬했을 때 값이 없는 행성이 결과 맨 위를 채우는 문제가 생긴다.

오름차순·내림차순 어느 쪽이든 NULL은 항상 목록 맨 뒤로 보낸다. 값을 임의로 채우지 않으면서도(문서 02, 1.1절 NULL 보존 원칙), 실제로 쓸모 있는 결과를 앞쪽에 배치하기 위함이다.
...
```
`message`는 모든 400 응답이 공유하는 공용 문구다(`exception_handler.py`의 `DEFAULT_MESSAGE_BY_STATUS`). 엔드포인트마다 다른 문구를 쓰려면 핸들러에 예외 케이스를 추가해야 하는데, 이는 "모든 400 응답이 같은 모양"이라는 공용 봉투의 원칙을 흐리므로 하지 않는다. 실제로 무엇이 잘못됐는지는 `fields`로 충분히 전달된다.
...
> **이 응답은 자주 바뀌지 않으므로 서버에서 캐싱한다.** 1시간 TTL로 Django 기본 캐시(`cache.get`/`cache.set`)를 직접 사용한다.
>
> 설계 당시엔 `cache_page(60 * 60)` 데코레이터를 고려했으나, 구현 단계에서 직접 캐싱으로 변경했다. 이유는 세 가지다.
> 1. `cache_page`는 원래 Django의 HTML 뷰를 전제로 만들어져, DRF의 `Response` 객체와 렌더링 시점이 어긋나 우회 코드가 필요해진다.
> 2. `neo_fetch_log`로 캐시 유무를 코드에서 눈으로 확인할 수 있게 만든 것과 같은 원칙 — 캐시 히트/미스가 데코레이터 안에 숨지 않고 뷰 코드에 그대로 드러난다.
> 3. `cache.get()` 한 줄이 곧 이 API가 하는 일을 보여주는 문서 역할을 한다.
>
> 참고: 이 캐시는 DRF `ScopedRateThrottle`이 요청 횟수를 세는 것과 같은 Django 기본 캐시(`LocMemCache`)를 공유한다. 프로세스별로 독립돼 있어(`runserver`와 `manage.py shell`은 서로 다른 메모리를 본다), 값이 안 보인다고 캐싱이 실패한 게 아닐 수 있다 — 같은 프로세스 안에서 확인해야 한다.
...
```json
{ "error": { "code": "ALREADY_EXISTS", "message": "이미 관심 목록에 등록된 항목입니다." } }
```
...
> 존재하지 않는 `nasa_id`로 저장을 시도한 경우다. 최초 설계에 없던 케이스였는데, `5.2`·`6.2` 등 프로젝트 전반이 "리소스 없음 = 404"로 일관되어 있어 그 관례를 그대로 확장하기로 확정했다(M2 구현 중 결정. `400`으로 처리하는 방식도 검토했으나, 최적이 아니라 기존 코드베이스와의 일관성을 기준으로 기각).
...
`12`, `13`, `14`번 항목(3장 엔드포인트 목록)과 동일한 구조다.
...
```json
{ "exoplanet_id": 1042, "saved_at": "2026-08-21T09:30:00Z" }
```
...
```json
{ "error": { "code": "NOT_FOUND", "message": "해당 외계행성을 찾을 수 없습니다." } }
```
 
**`DELETE` 응답 `200`**
 
```json
{ "exoplanet_id": 1042, "deleted": true }
```
 
> POST/DELETE 응답의 식별자 키는 `exoplanet_id`로 통일한다. 목록(GET) 응답의 `id`는 외계행성 자신의 필드(`6.1`/`6.2`와 같은 관례)이지만, 등록·해제 응답은 요청 본문 키(`exoplanet_id`)를 그대로 돌려주는 게 `7.3` NEO Watchlist가 요청·응답 전 구간에서 `nasa_id`로 통일된 것과 같은 원칙이다. 최초 설계에 없던 구멍이었고 M2 구현 중 확정했다.
