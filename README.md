...
> **개발 진행 중** — 백엔드 API 17개 전부 구현 완료 (M2), 프론트엔드(React) M3 진행 중 (UI 목업 + API 클라이언트 완료 → 라우팅·화면 구현 진행 중)
...
각 마일스톤 착수 시점에 Draft PR을 열고, 진행 중인 작업 내용·트러블슈팅·완료 기준을 세션마다 갱신했다. 마일스톤이 끝나면 `docs/05_milestones.md`의 완료 기준 체크리스트가 전부 충족된 상태로 PR을 Ready for review로 전환한 뒤 병합했다. 1인 프로젝트라 실질적인 코드 리뷰는 아니지만, **각 PR이 "무엇을 검증하고 병합했는지"의 기록**으로 남도록 하기 위함이다. (M0~M2는 마일스톤 완료 후 PR을 여는 방식이었으나, M3부터 착수 시점에 Draft로 여는 방식으로 변경했다 — Draft 상태에선 병합 버튼이 잠겨 미완성 코드가 실수로 `main`에 올라가는 것을 막아준다.)
